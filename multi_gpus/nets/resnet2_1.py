...
def build_net(x, is_training, FLAGS):
    ...
    with tf.variable_scope('pre'):
        pre = layers.conv(x, num_outputs=16,  kernel_size = [3, 3], scope='conv1', b_norm=True, is_training=is_training,
                          weight_decay=FLAGS.weight_decay)
        pre = layers.conv(pre, num_outputs=32,  kernel_size = [3, 3], scope='conv2', b_norm=True, is_training=is_training,
                          weight_decay=FLAGS.weight_decay)
    ...
    h = layers.conv(h, num_outputs=64, kernel_size=[1, 1], scope='fc1', padding='VALID',
                    b_norm=True, is_training=is_training, weight_decay=FLAGS.weight_decay, activation_fn=None)
    
    h = layers.conv(h, num_outputs=FLAGS.num_classes, kernel_size=[1, 1], scope='fc2', padding='VALID',
                    b_norm=True, is_training=is_training, weight_decay=FLAGS.weight_decay, activation_fn=None)
