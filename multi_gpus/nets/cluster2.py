...
def build_net(x, is_training, FLAGS):
    ...
    con = tf.reshape(con, [-1, 1, num_branches*128])

    w = layers.conv(y4, num_outputs=FLAGS.num_classes*num_branches*128, kernel_size=[1, 1], scope='fc2', padding='VALID',
                    b_norm=True, is_training=is_training, weight_decay=FLAGS.weight_decay, activation_fn=None)
    w = tf.reshape(w, [-1, num_branches*128, FLAGS.num_classes])
