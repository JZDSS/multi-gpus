...
def build_net(x, is_training, FLAGS):
    ...
    for i in range(1, n + 1):
        tmp, h = block(h, 32, FLAGS.weight_decay, '32_block{}'.format(i), is_training)

    shape = tmp.get_shape().as_list()

    pool1 = tf.contrib.layers.avg_pool2d(tmp, [shape[1], shape[2]], scope='global_pool1')
    ...
    _, h = block(h, 64, FLAGS.weight_decay, '64_block1', is_training, True)
    for i in range(2, n + 1):
        tmp, h = block(h, 64, FLAGS.weight_decay, '64_block{}'.format(i), is_training)
    
    shape = tmp.get_shape().as_list()

    pool2 = tf.contrib.layers.avg_pool2d(tmp, [shape[1], shape[2]], scope='global_pool2')
    fc2 = layers.conv(pool2, num_outputs=FLAGS.num_classes, kernel_size=[1, 1], scope='fc2', padding='VALID',
                    b_norm=True, is_training=is_training, weight_decay=FLAGS.weight_decay, activation_fn=None)

    _, h = block(h, 128, FLAGS.weight_decay, '128_block1', is_training, True)
    for i in range(2, n + 1):
        _, h = block(h, 128, FLAGS.weight_decay, '128_block{}'.format(i), is_training)
