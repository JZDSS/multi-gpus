...
flags.DEFINE_integer('max_steps', 128000, 'max steps')
...
def main(_):
    ...
    with tf.device('/cpu:0'):
        ...
        learning_rate = tf.train.exponential_decay(0.01, global_step, 2000, 0.8, staircase=True)
        ...
        images_v, labels_v = input_pipeline(
            tf.train.match_filenames_once(os.path.join(FLAGS.data_dir, 'valid', '*.tfrecords')), 128//num_gpus)
        ...
        for i in range(num_gpus):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('tower_%d' % i) as scope:
                    ...
                    correct_prediction = tf.equal(tf.reshape(tf.argmax(logits_v, 1), [-1, 1]), tf.cast(label_batch_v, tf.int64))
                    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
        ...
        summary_op = tf.summary.merge_all()
        summ_mean = tf.summary.merge(tf.get_collection('mean'))
        summ_var = tf.summary.merge(tf.get_collection('var'))
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(FLAGS.start_step, FLAGS.max_steps + 1):
                ...
                sum_mean, sum_var = sess.run([summ_mean, summ_var], feed_dict={is_training: False})
                train_writer.add_summary(sum_mean, i)
                train_writer.add_summary(sum_var, i)
                if i % d == 0:  # Record summaries and test-set accuracy
                    ...
                    acc, loss, summ, lr, acc_v = sess.run([accuracy, batch_loss, summary_op, learning_rate, accuracy_v], feed_dict={is_training: False})
                    cache[int(i/d)%5] = acc
                    cache_v[int(i/d)%5] = acc_v
                    train_writer.add_summary(summ, i)
                    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), file=f)
                    print('step %d: acc(t)=%f(%f), loss=%f; acc(v)=%f(%f); lr=%e' % (i, acc, cache.mean(), loss, acc_v, cache_v.mean(), lr), file=f)
