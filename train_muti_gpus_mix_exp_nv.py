...
def main(_):
    ...
    with tf.device('/cpu:0'):
        ...
        # learning_rate = tf.train.exponential_decay(0.1, global_step, 1500, 0.91, staircase=True)
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(FLAGS.start_step, FLAGS.max_steps + 1):
                ...
                if i % d == 0:  # Record summaries and test-set accuracy
                    ...
                    acc, loss, summ, lr = sess.run([accuracy, batch_loss, summary_op, learning_rate], feed_dict={is_training: False})
                    cache[int(i/d)%5] = acc
                    train_writer.add_summary(summ, i)
                    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), file=f)
                    print('step %d: acc(t)=%f(%f), loss=%f; lr=%e' % (i, acc, cache.mean(), loss, lr), file=f)
